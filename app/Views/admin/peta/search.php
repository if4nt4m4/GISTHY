...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">        
            ...
            <!-- Form Pencarian -->
            <form action="<?= base_url('admin/searchPeta') ?>" method="get" style="max-width: 400px;">
                <div class="input-group" style="position: relative;">
                    <input type="text" id="searchInput" name="searchInput" class="form-control" placeholder="Cari Wilayah/Kecamatan/Kelurahan" value="<?= $searchKeyword ?? '' ?>" autocomplete="off">
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            ...
            <div id="map" style="height: 600px;"></div> <!-- Pastikan peta memiliki tinggi yang cukup -->
        </div>
    </div>
</div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        var recommendationItems = document.getElementById("recommendationItems");

        // Gunakan seluruh data tanah untuk rekomendasi
        var rekomendasi = <?= json_encode($allTanahData); ?>;

        if (!searchInput || !recommendationList || !recommendationItems) {
            console.error("Elemen input atau rekomendasi tidak ditemukan.");
            return;
        }
        ...
        searchInput.addEventListener("input", function () {
            ...
            if (query.length > 0) {
                ...
                rekomendasi.forEach(function (item) {
                    ...
                    if (item.wilayah && item.wilayah.toLowerCase().includes(query)) {
                        wilayahSet.add(item.wilayah);
                    }
                    if (item.kecamatan && item.kecamatan.toLowerCase().includes(query)) {
                ...
                function addCategoryItems(categoryName, items) {
                    if (items.size > 0) {
                        var categoryTitle = document.createElement("li");
                ...
                addCategoryItems("Wilayah", wilayahSet);
                addCategoryItems("Kecamatan", kecamatanSet);
                addCategoryItems("Kelurahan", kelurahanSet);
    ...
    // Pastikan hargaMin dan hargaMax didefinisikan sebelum digunakan dalam legend
